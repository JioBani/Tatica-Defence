...
namespace Scenes.Battle.Feature.Units.Damage
{
    /// <summary>
    /// 기획서 1.3 데미지 계산 공식.
    /// 순서: 기본 데미지 → 치명타 → 방어력 → 피해 증가 → 소수점 버림(최소 0)
    /// 단일 공격력·방어력만 사용한다(물리/마법 구분 없음).
    /// </summary>
    public static class DamageCalculator
    {
            ...
            float skillCoefficient = 1f)
        {
            ...
            return Calculate(attackerStats, victimStats, isCritical, skillCoefficient);
        }
        ...
        {
            // 1. 기본 데미지 (단일 공격력)
            float baseDamage = attackerStats.Attack.CurrentValue;
            ...
            // 3. 방어력 적용 (단일 방어력)
            float defense = victimStats.Defense.CurrentValue;
        ...
        /// <summary>
        /// 사전 계산된 baseDamage를 받아 치명타/방어/피해증가 파이프라인을 적용한다.
        ...
        {
            bool isCritical = Random.value < attackerStats.CriticalChance.CurrentValue;
            return Calculate(baseDamage, attackerStats, victimStats, isCritical);
        }
